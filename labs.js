...
SG.LABS.cz = {
  ...
  open(host, ctx) {
    ...
    const vSlider = el('input', { type: 'range', min: 0.3, max: 2.0, step: 0.01, value: 1.0 });
    const pSlider = el('input', { type: 'range', min: -5, max: 5, step: 0.1, value: 0 });
    const vOut = el('b', null, '1.00'); const pOut = el('b', null, '+0.0');
    ...
    function draw() {
      ...
      const v = Number(vSlider.value); const vOk = v >= 0.7 && v <= 1.3;
    ...
    function physics(dt) {
      ...
      profile.push([len, D]);
      samples.push([Math.abs(D - 305) <= 3 ? 1 : 0, (v >= 0.7 && v <= 1.3) ? 1 : 0]);
    }
    function step(now) {
...
SG.LABS.reticle = {
  ...
  open(host, ctx) {
    ...
    const model = el('select', null, [['nb', 'Negative binomial (α = 3, industry default)'], ['murphy', 'Murphy'], ['poisson', 'Poisson (most pessimistic)']].map(([v, t]) => el('option', { value: v, selected: v === d.model ? '' : null }, t)));
    ...
    host.append(
      ...
      el('div', { class: 'design-grid' },
        el('div', null,
          ...
          el('label', null, 'What-if D0 (/cm²): ', el('b', { id: 'dD0' }), d0S, el('button', { class: 'btn ghost small', onclick: () => { d0S.value = currentD0().toFixed(3); render(); } }, 'use current D0')),
...
SG.LABS.oxide = {
  ...
  open(host, ctx) {
    ...
    const k = 8.617e-5;
    const TARGETS = [
      { name: 'Pad oxide under the nitride', nm: 8 }, { name: 'Tunnel oxide (flash)', nm: 3 }, { name: 'STI liner', nm: 15 }, { name: 'Field oxide', nm: 500 }
    ];
    ...
    host.append(
      ...
      el('p', null, 'Thermal oxidation follows x² + A·x = B·(t + τ). Thin oxide grows linearly (the surface reaction limits it); thick oxide grows as √t (oxygen must diffuse through the oxide already there, so doubling the thickness takes four times as long). Wet oxidation is much faster than dry but gives a rougher, less dense film. Hit each target within ±5% with the least furnace time.'),
...
SG.LABS.hbm = {
  ...
  open(host, ctx) {
    ...
    const BONDS = [
      { name: 'TC-NCF', bond: 0.97, cost: 40, note: 'thermocompression with a film per layer' }, { name: 'MR-MUF', bond: 0.985, cost: 60, note: 'mass reflow then molded underfill' }, { name: 'Hybrid bonding (Cu–Cu)', bond: 0.995, cost: 150, note: 'no solder; required for 16-high at the 720 µm cap' }
    ];
    ...
    function render() {
      ...
      const invalid = height === 16 && b.name !== 'Hybrid bonding (Cu–Cu)';
      const cfg = { height, escape: t.escape, bond: b.bond }; const Y = invalid ? 0 : hbmStackYield(cfg);
      ...
      out.innerHTML = invalid ? `<b class="warn">A 16-high stack does not fit the 720 µm JEDEC height with solder microbumps.</b> Dies would have to be ~20–25 µm thin and bonded without solder: choose hybrid bonding.` :
        ...
        <tr><td>Stack yield</td><td><b>${pct(Y)}</b> <span class="muted">= ${(1 - t.escape).toFixed(2)}^${height + 1} × ${b.bond}^${height} = ${pct(Math.pow(1 - t.escape, height + 1))} × ${pct(Math.pow(b.bond, height))}</span></td></tr>
...
SG.LABS.test = {
  ...
  open(host, ctx) {
    ...
    const LATENT = 0.03, AF = 78, TAU = 700, FIELD = 100000;
    ...
    host.append(
      ...
      el('p', null, `About ${pct(LATENT)} of packaged GPUs carry a latent defect that ATE cannot see. Burn-in at 125 °C ages a part ≈78× faster than 55 °C service (Ea = 0.7 eV), so 10 h ≈ 780 field hours; the fraction of infant-mortality failures forced out is 1 − e^(−h·78/700). Every hour also costs money and slightly overstresses good parts. An escape that fails in the field costs ${fmt$(FIELD)} (the rule-of-ten replacement plus an interrupted training job). Find the cheapest flow per unit.`),
