...
(function () {
  ...
  window.SG_CHALLENGES = {
    ...
    'dram-cell': { goal: 'Raise the read signal to 160 mV or more, comfortably above the sense amplifier\'s ~20–40 mV offset, and note whether you did it with the cell capacitor or with a shorter bit line.', check(box, u) { const v = u.statNum(box, /read signal/i); return { done: v >= 160, detail: `${v} mV` }; } },
    ...
    'yield-cascade': { goal: 'Reproduce the course\'s 0.95 × 0.6 × 0.98 × 0.97 ≈ 0.54, then find the one term that moves the product most (die yield).', check(box, u) { const y = u.statNum(box, /compounded yield/i); const sort = Number(u.ctlVal(box, /wafer sort/i)); return { done: sort <= 60 && y > 0, detail: `${y}% at sort ${sort}%` }; } },
